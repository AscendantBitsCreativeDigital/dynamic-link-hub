...
render a vertical stack of buttons:

1. A button that always links to your most recently published post
   (label and on/off are configurable).
2. Any number of custom buttons you add, each linking to an existing page
   or post, or to a custom URL — add, edit, remove, and drag to reorder
   from **Link Hub** in the admin menu.
3. A horizontally centered row of large, official-brand-color social
...
Button color, text color, hover colors, and corner radius are all
configurable under the **Appearance** tab.
